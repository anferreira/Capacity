...
private const string DATA_CMSLIBRARY_3rd	    = "DataBase.CMSLibrary_3rd";
...
private static string   cMSLIBRARY_3rd;
...
private static string startMajorGroup;
private static string endMajorGroup;
private static string wipStartMajorGroup;
private static string wipEndMajorGroup;
private static string report1268ShowOnlyWithDemand;
private static string report1268ExportPath;

private static string sdataTransferMaxRecords;
private static string sdataTransBillToOrdDateSameDateReq;

private static string sdataTransOcritExcludeUserIds;
private static string sdataTransOcrrtExcludeUserIds;
...
public static 
string CMSLibrary_3rd{
	get{
		return cMSLIBRARY_3rd;
	}
}
...
public static 
int SearchDefaultMaxRecordsInt{
	get{
        int irecords = NumberUtil.isIntegerNumber(searchDefaultMaxRecords) ? Convert.ToInt32(searchDefaultMaxRecords) :50;
		return irecords;
	}
}
...
public static 
string StartMajorGroup{
	get{
		return startMajorGroup;
	}
}

public static 
bool Report1268ShowOnlyWithDemand{
	get{
        if (report1268ShowOnlyWithDemand.ToUpper().Equals("Y"))
		    return true;
		return false;        
	}
}
...
public static 
string Report1268ExportPath{
	get{
		return report1268ExportPath;
	}
}

public static 
int DataTransferMaxRecords{
	get{
        if (NumberUtil.isIntegerNumber(sdataTransferMaxRecords))
		    return Convert.ToInt32(sdataTransferMaxRecords);
		return 0;        
	}
}

public static 
string DataTransBillToOrdDateSameDateReq{
	get{       
		return sdataTransBillToOrdDateSameDateReq;		
	}
}

public static 
string DdataTransOcritExcludeUserIds{
	get{       
		return sdataTransOcritExcludeUserIds;		
	}
}

public static 
string DdataTransOcrrtExcludeUserIds{
	get{       
		return sdataTransOcrrtExcludeUserIds;		
	}
}

        
public static 
string EndMajorGroup{
	get{
		return endMajorGroup;
	}
}

public static 
string WipStartMajorGroup{
	get{
		return wipStartMajorGroup;
	}
}

public static 
string WipEndMajorGroup{
	get{
		return wipEndMajorGroup;
	}
}
...
void OnApplicationStart(){	
	...
	if (!blnDefault && File.Exists(strFilePath)){
		try{
            ...
            cMSLIBRARY_3rd = ReadXmlSetting(root,nsmgr,"DataBase.CMSLibrary_3rd",DATA_CMSLIBRARY_3rd);
            ...
            startMajorGroup = ReadXmlSetting(root, nsmgr, "Report.StartMajorGroup", "");
            endMajorGroup = ReadXmlSetting(root, nsmgr, "Report.EndMajorGroup", ""); 

            wipStartMajorGroup = ReadXmlSetting(root, nsmgr, "Report.WipStartMajorGroup", "");
            wipEndMajorGroup = ReadXmlSetting(root, nsmgr, "Report.WipEndMajorGroup", ""); 

            report1268ShowOnlyWithDemand= ReadXmlSetting(root, nsmgr, "Report.ShowOnlyWithDemand", "");
            report1268ExportPath = ReadXmlSetting(root, nsmgr, "Report.ExportPath", "");

            sdataTransferMaxRecords             = ReadXmlSetting(root, nsmgr, "DataTransfer.MaxRecords","0");
            sdataTransBillToOrdDateSameDateReq  = ReadXmlSetting(root, nsmgr, "DataTransfer.BillToOrdDateSameDateReq","");            
            sdataTransOcritExcludeUserIds       = ReadXmlSetting(root, nsmgr, "DataTransfer.OcritExcludeUserIds", "");  
            sdataTransOcrrtExcludeUserIds       = ReadXmlSetting(root, nsmgr, "DataTransfer.OcrrtExcludeUserIds", "");  
...
public
static bool valueInList(string svalueSearch,string slist,char cep=','){
    bool        bresult =false;
    string[]    sitems  = slist.Split(cep);
    string      saux    = "";    

    for (int i=0; i < sitems.Length && !bresult; i++){
        saux = sitems[i];
        if (saux.ToUpper().Equals(svalueSearch.ToUpper()))             
            bresult = true;        
    }
    return bresult;   	
}
...
public
static string sqlListString(string slist,char cep=','){    
    string[]    sitems  = slist.Split(cep);
    string      saux    = "";    
    string      sql     = "";
    ...
    for (int i=0; i < sitems.Length; i++){
        saux = "'" + sitems[i] + "'";                      
        sql+= string.IsNullOrEmpty(sql) ? saux : "," + saux;
    }
    return sql;   	
}
