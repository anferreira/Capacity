...
namespace Nujit.NujitERP.ClassLib.Common
{
	...
	public class StringUtil
	{
        ...
        public static 
        bool ifMatch(string svalue,string smatch,char cwildCard){            
            Boolean bresult = Regex.IsMatch(svalue, wildCardToRegular(smatch.Replace(cwildCard,'*')));
            return (bool)bresult;
        }

        public static 
        bool ifMatch(string svalue,string smatch){            
            Boolean bresult = ifMatch(svalue,smatch,'%');
            return (bool)bresult;
        }
}
}
