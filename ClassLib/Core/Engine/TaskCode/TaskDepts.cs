...
public 
class TaskDepts : GenericTaskCode{

public
TaskDepts(TaskNode taskNode, string parameters) : base(taskNode, parameters){
}
...
void run(){
	...
	try{
		...
		log(coreFactory, task, Task.TASK_RUNNING, "Copying Depts");

		int copied = coreFactory.generateCMSDeptsRecords().Length;
		
		task.setAdicionalData("copied : " + copied.ToString() + " records of depts table");
		Engine.getInstance(coreFactory).finishTask(taskNode, Task.TASK_FINISHED);
