...
public 
class TaskFamilyParts : GenericTaskCode{

public
TaskFamilyParts(TaskNode taskNode, string parameters) : base(taskNode, parameters){
}
...
void run(){
	...
	try{
		...
		log(coreFactory, task, Task.TASK_RUNNING, "Copying Family");

		int copied = coreFactory.CMSFamilyCopy();
		
		task.setAdicionalData("copied : " + copied.ToString() + " records of family table");
