...
public 
class TaskLbhist : GenericTaskCode{

public
TaskLbhist(TaskNode taskNode, string parameters) : base(taskNode, parameters){
}
...
void run(){
	...
	try{
		...
		log(coreFactory, task, Task.TASK_RUNNING, "Copying Lbhist");

		int copied = coreFactory.generateCmsLbHist();
		
		task.setAdicionalData("copied : " + copied.ToString() + " records of lbhist table");
