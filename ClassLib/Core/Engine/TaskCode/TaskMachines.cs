...
public 
class TaskMachines : GenericTaskCode{

public
TaskMachines(TaskNode taskNode, string parameters) : base(taskNode, parameters){
}
...
void run(){
	...
	try{
		...
		log(coreFactory, task, Task.TASK_RUNNING, "Copying Sprsn");

		int copied = coreFactory.generateCMSMachineRecords();
		
		task.setAdicionalData("copied : " + copied.ToString() + " records of sprsn table");
