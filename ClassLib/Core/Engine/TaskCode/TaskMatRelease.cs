...
public 
class TaskMatRelease : GenericTaskCode{

public
TaskMatRelease(TaskNode taskNode, string parameters) : base(taskNode, parameters){
}
...
void run(){
	...
	try{
		...
		log(coreFactory, task, Task.TASK_RUNNING, "Copying DelFor");

		int copied = coreFactory.generateRRLToDelFor();
		
		task.setAdicionalData("copied : " + copied.ToString() + " records of delfor table");
