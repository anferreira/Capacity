...
public 
class TaskPrHist : GenericTaskCode{

public
TaskPrHist(TaskNode taskNode, string parameters) : base(taskNode, parameters){
}
...
void run(){
	...
	try{
		...
		log(coreFactory, task, Task.TASK_RUNNING, "Copying PrHist");

		int copied = coreFactory.generateCMSPrhist();
		
		task.setAdicionalData("copied : " + copied.ToString() + " records of prhist table");
