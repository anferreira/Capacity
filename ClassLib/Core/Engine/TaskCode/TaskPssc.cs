...
public 
class TaskPssc : GenericTaskCode{

public
TaskPssc(TaskNode taskNode, string parameters) : base(taskNode, parameters){
}
...
void run(){
	...
	try{
		...
		log(coreFactory, task, Task.TASK_RUNNING, "Copying Pssc");

		//int copied = coreFactory.cms400ToNujitPssc();
		int copied = coreFactory.cms400ToCmsTempPssc();	
		
		task.setAdicionalData("copied : " + copied.ToString() + " records of pssc table");
