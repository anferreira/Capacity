...
public 
class TaskScrap : GenericTaskCode{

public
TaskScrap(TaskNode taskNode, string parameters) : base(taskNode, parameters){
}
...
void run(){
	...
	try{
		...
		log(coreFactory, task, Task.TASK_RUNNING, "Copying Scprap");

		int copied = coreFactory.cms400ToNujitScrap();
		
		task.setAdicionalData("copied : " + copied.ToString() + " records of scrap table");
