...
public 
class TaskStkr : GenericTaskCode{

public
TaskStkr(TaskNode taskNode, string parameters) : base(taskNode, parameters){
}
...
void run(){
	...
	try{
		...
		log(coreFactory, task, Task.TASK_RUNNING, "Copying Stkr");

		int copied = coreFactory.generateCMSStkr();
		
		task.setAdicionalData("copied : " + copied.ToString() + " records of stkr table");
