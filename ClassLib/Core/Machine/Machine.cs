...
Machine(CapMacCfgDataBaseContainer capMacCfgDataBaseContainer,
    ...
    this.scheduled = Constants.STRING_YES;
    this.priorityShow = 0;
    ...
    this.machineDef = null;
}

public
Machine(int id,string plt, string dept, string mach, string des1,
		...
		string defSpaceUom, decimal maxWgt, string maxWgtUom,decimal directHoursToShifts,string scheduled){
...
[System.Runtime.Serialization.DataMember]
public
decimal DefSpaceRack {
	get { return defSpaceRack; }
	set { if (this.defSpaceRack != value){
			this.defSpaceRack = value;
			//Notify("DefSpaceRack");
		}
	}
}
...
public
string DefSpaceUom {
	get { return defSpaceUom; }
	set { if (this.defSpaceUom != value){
			this.defSpaceUom = value;
			//Notify("DefSpaceUom");
		}
	}
}
...
public
string MaxWgtUom {
	get { return maxWgtUom; }
	set { if (this.maxWgtUom != value){
			this.maxWgtUom = value;
			//Notify("MaxWgtUom");
		}
	}
}
...
string PlantDept{
	get {
        string saux = string.IsNullOrEmpty(plt) ? "" : plt;
        saux+= '/' + (string.IsNullOrEmpty(dept)? "" : dept);
        return saux; }
	set { 		
	}
}
...
MachineDef MachineDef {
	get { return machineDef; }
	set { if (this.machineDef != value){
			this.machineDef = value;			
		}
	}
}
...
public
int PriorityShow {
	get { return priorityShow; }
	set { if (this.priorityShow != value){
			this.priorityShow = value;			
		}
	}
}
...
bool Equals(object obj){
	if (obj is Machine)
		return	this.id.Equals(((Machine)obj).Id);
	else
		return false;
}
...
decimal getHrsPerShift(){
    decimal dhrsPerShift = machineDef != null ? machineDef.HrsPerShift : 0;

    if (dhrsPerShift <= 0)
        dhrsPerShift = (decimal)7.25;
    return dhrsPerShift;
}

public
decimal getStdShiftPerWeek(){
    decimal dstdShiftPerWeek = machineDef != null ? machineDef.StdShiftPerWeek : 0;

    if (dstdShiftPerWeek <= 0)
        dstdShiftPerWeek = (5*3);
    return dstdShiftPerWeek;
}

public
decimal getTotalWeekHrs(){
    decimal dtotal = getHrsPerShift() * getStdShiftPerWeek();    
    return dtotal;
}
