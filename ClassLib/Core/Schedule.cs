	...
	for(int i = 0; i < vec.Length; i++){
		...
		for(int z = 0; z < materials.Length; z++){
			...
			if (schMatReqDayDataBase == null){
				Inventory inventory = coreFactory.readInventory(materials[z][0],plt);
				...
				schMatReqDayDataBase.setSMD_InvQty(inventory!= null ?inventory.getTotalInventory(int.Parse(materials[z][1])):0);
