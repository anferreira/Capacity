...
Hashtable readPrHistByFiltersHashByPartSeq(string part,int seq,string sdept,DateTime fromDate,DateTime toDate,int irows){    
	try{            
        ...
        PrHistContainer         prHistContainer = new PrHistContainer();
        PrHist                  prHist = null;
        string                  skey="";
        PrhistDataBaseContainer prhistDataBaseContainer = new PrhistDataBaseContainer(dataBaseAccess);
        prhistDataBaseContainer.readByFilters(part,seq,sdept,fromDate,toDate,irows);

        foreach(PrhistDataBase prhistDataBase in prhistDataBaseContainer){
            prHist = objectDataBaseToObject(prhistDataBase);

            skey = prHist.DWPART.ToUpper() + Constants.DEFAULT_SEP + Convert.ToInt32(prHist.DWSEQN).ToString();
            if (hashtable.Contains(skey))
                prHistContainer = (PrHistContainer)hashtable[skey];
            else{
                prHistContainer = new PrHistContainer();
                hashtable.Add(skey,prHistContainer);
            }
            prHistContainer.Add(prHist);
        }
...
Hashtable readPrHistByFiltersHashSummarizedByPartSeq(string part,int seq,string sdept,DateTime fromDate,DateTime toDate,PrHistSumViewContainer prHistSumViewContainer, int irows){    
	try{            
        ...
        PrHistSumView           prHist = null;
        PrHistSumView           prHistNew = null;
        string                  skey="";
        PrhistSumViewSumViewDataBaseContainer prhistSumViewSumViewDataBaseContainer = new PrhistSumViewSumViewDataBaseContainer(dataBaseAccess);
        prhistSumViewSumViewDataBaseContainer.readByFilters(part,seq, sdept,fromDate, toDate, prHistSumViewContainer,irows);

        foreach (PrhistSumViewDataBase prhistSumViewDataBase in prhistSumViewSumViewDataBaseContainer){
            prHist = objectDataBaseToObject(prhistSumViewDataBase);

            skey = prHist.DWPART.ToUpper() + Constants.DEFAULT_SEP + Convert.ToInt32(prHist.DWSEQN).ToString();
            if (hashtable.Contains(skey))
                prHistNew = (PrHistSumView)hashtable[skey];
            else{
                prHistNew = new PrHistSumView();
                hashtable.Add(skey, prHistNew);
            }
            
            prHistNew.DWQTYC+= prHist.DWQTYC;
            prHistNew.DWQTYS+= prHist.DWQTYS;
        }
...
private
PrHist objectDataBaseToObject(PrhistDataBase prhistDataBase){
	PrHist prHist = new PrHist();

	prHist.DWDATE = prhistDataBase.getDWDATE();
	prHist.DWPART = prhistDataBase.getDWPART();
	prHist.DWSEQN = prhistDataBase.getDWSEQN();
    prHist.DWQTYC = prhistDataBase.getDWQTYC();
	prHist.DWQTYS = prhistDataBase.getDWQTYS();    

    return prHist;
}

private
PrHistSumView objectDataBaseToObject(PrhistSumViewDataBase prhistSumViewDataBase){
	PrHistSumView prHistSumView = new PrHistSumView();
	
	prHistSumView.DWPART = prhistSumViewDataBase.getDWPART();
	prHistSumView.DWSEQN = prhistSumViewDataBase.getDWSEQN();
    prHistSumView.DWQTYC = prhistSumViewDataBase.getDWQTYC();
	prHistSumView.DWQTYS = prhistSumViewDataBase.getDWQTYS();    

    return prHistSumView;
}

public
PrHistSumView createPrHistSumView(){
    return new PrHistSumView();
}

public
PrHistSumViewContainer createPrHistSumViewContainer(){
    return new PrHistSumViewContainer();
}
