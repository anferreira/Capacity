...
public 
class CapCfgShfDataBaseContainer : GenericDataBaseContainer{
...
public
CapCfgShfDataBaseContainer(DataBaseAccess dataBaseAccess) : base(dataBaseAccess){
}
...
void read(){
	...
	try{
		string sql = "select * from capcfgshf";

		reader = dataBaseAccess.executeQuery(sql);
		while(reader.Read()){
			CapCfgShfDataBase capCfgShfDataBase = new CapCfgShfDataBase(dataBaseAccess);
			capCfgShfDataBase.load(reader);
			this.Add(capCfgShfDataBase);
		}
...
void readByPltDeptMach(){
	NotNullDataReader reader = null;
	try{
		string sql = "select * from capcfgshf A, capmaccfga B where " +
			"B.CMCA_Plt = '" + plt + "' and " +
			"B.CMCA_Dept = '" + dept + "' and " +
			"B.CMCA_Mach = '" + mach + "' and " +
			"B.CMCA_Cfg = A.CCH_Cfg";
