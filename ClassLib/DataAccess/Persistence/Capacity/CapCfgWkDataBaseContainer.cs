...
public 
class CapCfgWkDataBaseContainer : GenericDataBaseContainer{
...
public
CapCfgWkDataBaseContainer(DataBaseAccess dataBaseAccess) : base(dataBaseAccess){
}
...
void read(){
	...
	try{
		string sql = "select * from capcfgwk";

		reader = dataBaseAccess.executeQuery(sql);
		while(reader.Read()){
			CapCfgWkDataBase capCfgWkDataBase = new CapCfgWkDataBase(dataBaseAccess);
			capCfgWkDataBase.load(reader);
			this.Add(capCfgWkDataBase);
		}
...
void readByPltDeptMach(){
	NotNullDataReader reader = null;
	try{
		string sql = "select * from capcfgwk A, capmaccfga B where " +
			"B.CMCA_Plt = '" + plt + "' and " +
			"B.CMCA_Dept = '" + dept + "' and " +
			"B.CMCA_Mach = '" + plt + "' and " +
			"B.CMCA_Cfg = A.CCW_Cfg";
