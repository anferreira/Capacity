...
" where  " +
    ...
    read(sql);
        ...
        */
private
string getCheckFESDATMinDate(){
    string sql = " FESDAT > '" + DateUtil.getDateRepresentation(DateUtil.MinValue,DateUtil.MMDDYYYY) + "' ";
    return sql;
}

private
string getSelectConcatOcrit(){
    ...
    return selectConcatOcrit;
}

private
string getSqlOcriBase(bool bapplyRule60Days){
    string sdateOcri    = "IFNULL(CAST(DC4TMSP AS Date),FESDAT)";   
    string sminPostDate = " FEFUTR is not null and length(FEFUTR) >= 8 and LOCATE('/', FEFUTR) >= 3 and " + sdateOcri + " <= CAST(REPLACE(Concat('20',substr(FEFUTR,1,8)), '/', '-') AS Date) ";
    string srule60DaysAndMinorPostDate  =" and " + getCheckFESDATMinDate() +  " and " + sdateOcri + " >= (FESDAT- 60 days) and " + sminPostDate;//sdateOcri + " <= FESDAT";
    string srule60Days  = " and " + getCheckFESDATMinDate() + " and " + sdateOcri + " >= (FESDAT- 60 days)";//sdateOcri + " <= FESDAT";
    string sexcludeUsers = getSqlOcriExcludedUsers("DC4USR");
   
    string selectConcatOcrit = "   Concat(DC4TMSP,  Concat('" + Constants.DEFAULT_SEP + "',"+ " Concat (Cast(DC4SDAT as varchar(30)),Concat ('" +  Constants.DEFAULT_SEP + "', Cast(DC4QDAT as varchar(30)) ))) )";    
    string sqlocrit =" select SELXXFIELD from " + getTablePrefix() + "ocrit as ot where ot.DC4ORD#= oi.DDORD# and ot.DC4ITM#= oi.DDITM# " +
                    ...
                    (bapplyRule60Days ? srule60Days : srule60DaysAndMinorPostDate) +
                    (string.IsNullOrEmpty(sexcludeUsers)? "" : " and " + sexcludeUsers);
...
string getSqlOcri(bool bapplyRule60Days){
    ...
    string sqlocrit =" select " + selectConcatOcrit + "DC4TMSP from " + getTablePrefix() + "ocrit as ot where ot.DC4ORD#= oi.DDORD# and ot.DC4ITM#= oi.DDITM# " +
                    ...
                    (bapplyRule60Days ? srule60Days : srule60DaysAndMinorPostDate) +
                    (string.IsNullOrEmpty(sexcludeUsers)? "" : " and " + sexcludeUsers)+
                    " order by DC4TMSP " + (bapplyRule60Days ? "" : "desc"); //2020-06-30 show newest so added desc
...
private
string getSqlOcriNew(bool bapplyRule60Days){
    string select       = "SELXXFIELD";
    string selectNewest = "DC4TMSP DC4TMSP2, DC4SDAT DC4SDAT2,DC4QDAT DC4QDAT2,DC4QTOI DC4QTOI2"; 
    string sqlNewest    = getSqlOcriBase(false).Replace(select,selectNewest);
    string sqlAnyChange = getSqlOcri(false);

    string selectChages = getSelectConcatOcrit(); 
    string sqlChanges   = getSqlOcriBase(false).Replace(select,selectChages);

    sqlNewest += " order by DC4TMSP desc";
    sqlNewest  = "(" + DBFormatter.selectTopRows(sqlNewest, 1) + ")";

    sqlChanges+= " and exists ( select DC4SDAT2 from " + sqlNewest + " as querynewest " +
                 " where  (ot.DC4SDAT <> DC4SDAT2 or ot.DC4QTOI <> DC4QTOI2) " +
                 " ) order by ot.DC4TMSP desc";

    sqlChanges = "(" + sqlChanges + ")";
    return sqlChanges;
}
...
string sqlOCRRT(){
    ...
    sqlOnRangeNew+= (string.IsNullOrEmpty(sexcludeUsers) ? "" : " and " + sexcludeUsers) +
                    " and " + getCheckFESDATMinDate() +  " and " + sdate +  " >= (FESDAT- 60 days) and " + sdate + " <= FESDAT order by orr.DC5TMSP";
...
private
string sqlOcritChangesNew(){
    string select           = getSelectConcatOcrit().Replace("DC4","ot.DC4");
    string sexcludeUsers1   = getSqlOcrrtExcludedUsers("ot.DC4USR");            
    string sexcludeUsers2   = getSqlOcrrtExcludedUsers("ot2.DC4USR");            
    string sexcludeUsers3   = getSqlOcrrtExcludedUsers("ot3.DC4USR");            
    string sql = "( " +

    " select " + select +
    " from " + getTablePrefix() + "ocrit as ot , " + getTablePrefix() + "ocrit as ot2 " +
    " where  ot.DC4ORD#=bd.FGORD# and ot.DC4ITM#=  bd.FGITEM" +
    "   and ot.DC4TMSP is not null and ot.DC4SDAT is not null and ot.DC4QDAT is not null  and length(ot.DC4TMSP) >= 8 and LOCATE('-',ot.DC4TMSP) >=2 and LOCATE('-',ot.DC4TMSP) > 4  " +
    "  and  FESDAT > '01/01/1901'  and IFNULL(CAST(ot.DC4TMSP AS Date),FESDAT) >= (FESDAT- 60 days) and  FEFUTR is not null and length(FEFUTR) >= 8 " +
    "  and LOCATE('/', FEFUTR) >= 3 and IFNULL(CAST(ot.DC4TMSP AS Date),FESDAT) <= CAST(REPLACE(Concat('20',substr(FEFUTR,1,8)), '/', '-') AS Date)  " +
    (string.IsNullOrEmpty(sexcludeUsers1) ? "" : " and " + sexcludeUsers1) +

    " and  ot2.DC4ORD#= ot.DC4ORD# and ot2.DC4ITM#= ot.DC4ITM# " +
    " and ot2.DC4TMSP is not null and ot2.DC4SDAT is not null and ot2.DC4QDAT is not null  and length(ot2.DC4TMSP) >= 8 and LOCATE('-',ot2.DC4TMSP) >=2 and LOCATE('-',ot2.DC4TMSP) > 4 " +
    " and  FESDAT > '01/01/1901'  and IFNULL(CAST(ot2.DC4TMSP AS Date),FESDAT) >= (FESDAT- 60 days) and  FEFUTR is not null and length(FEFUTR) >= 8 " +
    " and LOCATE('/', FEFUTR) >= 3 and IFNULL(CAST(ot2.DC4TMSP AS Date),FESDAT) <= CAST(REPLACE(Concat('20',substr(FEFUTR,1,8)), '/', '-') AS Date)  " +
    (string.IsNullOrEmpty(sexcludeUsers2) ? "" : " and " + sexcludeUsers2) +

    " and ot2.DC4TMSP in " +
     " ( " +
    " select ot3.DC4TMSP from " + getTablePrefix() + "ocrit as ot3  where ot3.DC4ORD#=bd.FGORD# and ot3.DC4ITM#= bd.FGITEM " +

    " and ot3.DC4TMSP is not null and ot3.DC4SDAT is not null and ot3.DC4QDAT is not null  and length(ot3.DC4TMSP) >= 8 and LOCATE('-',ot3.DC4TMSP) >=2 and LOCATE('-',ot3.DC4TMSP) > 4  "+
    " and  FESDAT > '01/01/1901'  and IFNULL(CAST(ot3.DC4TMSP AS Date),FESDAT) >= (FESDAT- 60 days) and  FEFUTR is not null and length(FEFUTR) >= 8 and LOCATE('/', FEFUTR) >= 3 and IFNULL(CAST(ot3.DC4TMSP AS Date),FESDAT) <= CAST(REPLACE(Concat('20',substr(FEFUTR,1,8)), '/', '-') AS Date) " +
    (string.IsNullOrEmpty(sexcludeUsers3) ? "" : " and " + sexcludeUsers3) +
    " order by  ot3.DC4TMSP desc fetch  first 1 rows only " +
    " )" +
    " and   (ot.DC4SDAT <> ot2.DC4SDAT or ot.DC4QTOI <> ot2.DC4QTOI) " +
    " order by  ot.DC4TMSP desc fetch  first 1 rows only " +
    " )";
    return sql;
}
...
void readByFilters(string splant,string stpartner, string sbillTo, string shipTo, string sbol,string sorder, string spo,string scustPO,string shipped,string sdocType,string srelease,decimal orderItem,bool blateOrders,string sppap,DateTime fromDate, DateTime toDate, int irows){           
    string sqlocrit     = getSqlOcri(false) + "as OCRITValues";
    string sqlocritAny  = getSqlOcri(true)+ "as OCRITValuesAny";
    string sqlocritChang= sqlOcritChangesNew()+ "as OCRITValuesChange";    
    string sfilters     = "";
    ...
    string sqlDC5QTOO2  = sqlOnRange;
    string sqlDC5SDAT   = sqlOnRange;
    string sqlDC5ClostPost= sqlOnRange;

    sqlOnRangeNew +=  " and " + getCheckFESDATMinDate() +  " and " + sdate +  " >= (FESDAT- 60 days) and " + sdate + " <= FESDAT order by orr.DC5TMSP";
    ...
    sqlDC5ClostPost +=  " and " + getCheckFESDATMinDate() + " and " + sdate +  " >= (FESDAT- 60 days) and " +
                    "  FEFUTR is not null and length(FEFUTR) >= 8 and LOCATE('/', FEFUTR) >= 3 and " + sdate + " <= CAST(REPLACE(Concat('20',substr(FEFUTR,1,8)), '/', '-') AS Date) " + " order by orr.DC5TMSP desc";
    sqlDC5ClostPost = "(" + DBFormatter.selectTopRows(sqlDC5ClostPost, 1) + ") as ";
    sqlDC5QTOO      = sqlDC5ClostPost + " DC5QTOO";
    sqlDC5SDAT      = sqlDC5ClostPost + " DC5SDAT";

    sqlDC5QTOO2+=   " and " + getCheckFESDATMinDate()  + " and " + sdate +  " >= (FESDAT- 60 days) and " + sdate + " <= FESDAT order by orr.DC5TMSP desc";
    sqlDC5QTOO2 = "(" + DBFormatter.selectTopRows(sqlDC5QTOO2, 1) + ") as DC5QTOO2";
...
" tp.SMTRPT," +
" tp.SMSTXL," +
    sqlocrit + "," +  sqlocritAny + "," + sqlocritChang + "," +
    ...
    sqlDC5QTOO2.Replace(selectField,"DC5QTOO") + "," +
    sqlDC5SDAT.Replace(selectField, "DC5SDAT") +
