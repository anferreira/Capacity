...
private decimal PDM_DirectHoursToShifts;
private string PDM_Scheduled;
...
void load(NotNullDataReader reader){
    this.PDM_ID = reader.GetInt32("PDM_ID");
    ...
    this.PDM_DirectHoursToShifts = reader.GetDecimal("PDM_DirectHoursToShifts");
    this.PDM_Scheduled = reader.GetString("PDM_Scheduled"); 
}

public
bool read(){
	string sql = "select * from pltdeptmach where " + getWhereCondition();            
	return read(sql);
}

public
bool readById(int id){
	string sql = "select * from pltdeptmach where PDM_ID = " + NumberUtil.toString(id);    
	return read(sql);
}
...
public override
void delete(){	
	string sql = "delete from pltdeptmach where " + getWhereCondition();		
	delete(sql);	
}
...
void update(){	
	string sql = "update pltdeptmach set " +
        ...
        "PDM_DirectHoursToShifts = " + NumberUtil.toString(PDM_DirectHoursToShifts) + ", " +
        "PDM_Scheduled = '" + Converter.fixString(PDM_Scheduled) + "' " +        
...
public override
void write(){	
	string sql = "insert into pltdeptmach(PDM_Plt,PDM_Dept,PDM_Mach,PDM_Des1,PDM_Des2,PDM_Des3,PDM_Des4,PDM_PltInvLoc,PDM_InOut,PDM_MachTyp,PDM_SchType,PDM_UtilPer,"+
        "PDM_InvDrFr,PDM_InvRecTo,PDM_CableLn,PDM_LnUom,PDM_Speed,PDM_MaxRacks,PDM_DefSpaceRack,PDM_DefSpaceUom,PDM_MaxWgt,PDM_MaxWgtUom,PDM_MachShort,PDM_DirectHoursToShifts,PDM_Scheduled) values('" + 
        ...
        NumberUtil.toString(PDM_DirectHoursToShifts) + ",'" +
        Converter.fixString(PDM_Scheduled) + "')";
            
        write(sql);
        this.setPDM_ID( this.dataBaseAccess.getLastId());//get autonumeric inserted)        
}
...
bool existsByMach(){	
	string sql ="select * from pltdeptmach " + 
		        " where PDM_Dept ='" + PDM_Dept + "' and PDM_Mach = '" + PDM_Mach + "'";
	return exists(sql);		
}

public
void setPDM_ID(int PDM_ID){
	this.PDM_ID = PDM_ID;
}
...
public
void setPDM_DirectHoursToShifts(decimal PDM_DirectHoursToShifts){
	this.PDM_DirectHoursToShifts = PDM_DirectHoursToShifts;
}

public
void setPDM_Scheduled(string PDM_Scheduled){
	this.PDM_Scheduled = PDM_Scheduled;
}
...
public
decimal getPDM_DirectHoursToShifts(){
	return PDM_DirectHoursToShifts;
}

public
string getPDM_Scheduled(){
    return PDM_Scheduled;
}
