...
string getCompleteDateRepresentation(DateTime date){
    ...
    if (date.Year < MinValue.Year) //AF 2017-11-09
        yearStr = MinValue.Year.ToString();
...
public static
DateTime decimalMinutesToHoursHHMM(decimal dminutes) {
    try{

    DateTime    dateTime = MinValue;
    string      stime = Convert.ToInt32(dminutes).ToString();
    ...
    if (dminutes > 0){
        ...
        if (stime.Length==6){
            dateTime = new DateTime(1901, 01, 01, Convert.ToInt32(stime.Substring(0, 2)), Convert.ToInt32(stime.Substring(2,2)), Convert.ToInt32(stime.Substring(4, 2))); 
        }
    }
    /*
    TimeSpan    span = TimeSpan.FromMinutes(Convert.ToDouble(dminutes));
    string      shour= span.Hours.ToString("00");
    string      smin = span.Minutes.ToString("00");   
...
public
static DateTime convertDecimalToDate(decimal date) {
 try {
    ...
    if (dateString.Equals("0")) {
        ...
        int month = int.Parse(dateString.Substring(offset + 2,2));
        int day = int.Parse(dateString.Substring(offset + 4,2));
        int year = int.Parse(yearstr + dateString.Substring(offset,2));
        try {
...
public static
DateTime getDateTimeFromAS4002Points(DateTime date, string time){
