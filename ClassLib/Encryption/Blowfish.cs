...
namespace Nujit.NujitERP.ClassLib.Encryption
{
	public class Blowfish
	{
			...
			ref uint unLoRef)
		{
			...
			// and use local references (faster, too) 
			...
			unHi ^= (((sbox1[(int)(unLo >> 24)] + sbox2[(int)((unLo >> 16) & 0x0ff)]) ^ sbox3[(int)((unLo >> 8) & 0x0ff)]) + sbox4[(int)(unLo & 0x0ff)]);

			// finalize and copy back

			uint unSwap = unLo;
			unLoRef = unHi ^ pbox[16];
			unHiRef = unSwap ^ pbox[17];
		}
