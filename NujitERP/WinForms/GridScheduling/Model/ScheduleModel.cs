...
//		}
		...
		private void explodeDemands()
		{
			...
			while (hasMore || oneMoreToGo)
			{
				...
				else
				{
					...
					{
						bom = core.makeBomFromProdIDAndSeq (demand.getProdID(), int.Parse(demand.getSeq()),Configuration.DftPlant);
		...
		private void loadQohFromDemand (IDemand demand)
		{
			...
			{
				decimal qoh = 0;
				if (core.existsInventory(demand.getProdID(),Configuration.DftPlant))
				{
					Inventory inventory = core.readInventory(demand.getProdID(), Configuration.DftPlant);
