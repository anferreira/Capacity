...
using System.IO;
using Nujit.NujitERP.WinForms.Reports.HotList;
...
void loadGridHotList(){
	lock(this){
		...
		string generated = "";
		string exploded = "";
        DateTime hotListCreated = this.lastHLCreated; 
		...
		if (!lastVersionCheckBox.Checked){
			...
			if (hotListVersionForm.DialogResult == DialogResult.OK){
                ...
                hotListCreated = DateUtil.parseDate(generated.Substring(0,10),DateUtil.MMDDYYYY);               
			}else{
				return;
			}
		}else{//AF get Hot List created form last processed
            string[] v = coreFactory.getHotListLogData();
			this.lastHLCreated = hotListCreated= DateUtil.parseCompleteDate(v[0],DateUtil.MMDDYYYY);
        }
       
		DataSet dsHotList = LoadHotListGrid.generateHotListDataSet(vecFilterDept, vecFilterPart, vecFilterMg, false, type, hlid, hotListCreated);
...
string[][] generateHotList(){
	...
	try{
		coreFactory.createHotList(stkbFilterAux, wipbFilterAux, badWipb,0,Configuration.DftPlant);
	}catch(NujitException nexc){
		FormException formException = new FormException(nexc);
		formException.ShowDialog();
	}
...
void demandButton_Click(object sender, System.EventArgs e){
	string[] vecFilterPart = loadFilterPartSql();
	string[][] vec = coreFactory.getDemandAsString(vecFilterPart, Configuration.DftPlant);
...
private
void loadHotListMenuItems(){    
    ContextMenu contextMenu = new ContextMenu();
    MenuItem    menuItem = new MenuItem();
    MenuItem    menuItemChild = new MenuItem();    

    menuItem = new MenuItem("Export To CSV");
    contextMenu.MenuItems.Add(menuItem);

    menuItemChild = new MenuItem("Hot List", exportToCSVItemButton_Click);
    menuItem.MenuItems.Add(menuItemChild);
            
    //menuItemChild = new MenuItem("1268 Report", exportToCSV1268ReportButton_Click);
    //menuItem.MenuItems.Add(menuItemChild);

    menuItemChild = new MenuItem("1268 Stored", exportToCSVStored1268ReportButton_Click);
    menuItem.MenuItems.Add(menuItemChild);


    menuItem = new MenuItem("1268 Report");
    contextMenu.MenuItems.Add(menuItem);

    //menuItemChild = new MenuItem("Show", run1268ReportButton_Click);
    //menuItem.MenuItems.Add(menuItemChild);

    menuItemChild = new MenuItem("Show Stored", run1268StoredReportButton_Click);
    menuItem.MenuItems.Add(menuItemChild);

    menuItemChild = new MenuItem("Process/Store", store1268ReportButton_Click);
    menuItem.MenuItems.Add(menuItemChild);

    //OEE    
    menuItem = new MenuItem("OEE", oeeButton_Click);
    contextMenu.MenuItems.Add(menuItem);
    
    gridHotList.ContextMenu = contextMenu;
}

private
void exportToCSVItemButton_Click(object sender, System.EventArgs e){
    exportModel.hotlistExportToCSV(coreFactory,this,gridHotList, getPartType());
}

private
void callNewHotLis1268Report(){
    exportModel.callBatchFile("HotListReports.exe", "-R 1268-3", false);
}
...
private
void exportToCSV1268ReportButton_Click(object sender, System.EventArgs e){
    exportModel.exportToCSV1268Report(coreFactory,this);    
}

private
void exportToCSVStored1268ReportButton_Click(object sender, System.EventArgs e){
    this.exportModel.exportToCSVStored1268Report(coreFactory, this,true,false,false);    
}

        
private
void run1268ReportButton_Click(object sender, System.EventArgs e){
    callNewHotLis1268Report();
}

private
void run1268StoredReportButton_Click(object sender, System.EventArgs e){            
    exportModel.callBatchFile("HotListReports.exe","-R 1268-ST", false);
}
      
private
void store1268ReportButton_Click(object sender, System.EventArgs e){
    store1268Report();
}

private
void oeeButton_Click(object sender, System.EventArgs e){
    oee();
}
...
void store1268Report(){
    try{        
        ...
        this.Cursor = System.Windows.Forms.Cursors.Default;
        MessageBox.Show("Report Generated and Stored Properly.");
