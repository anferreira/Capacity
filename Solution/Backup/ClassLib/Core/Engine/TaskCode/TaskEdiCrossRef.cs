...
public 
class TaskEdiCrossRef : GenericTaskCode{

public
TaskEdiCrossRef(TaskNode taskNode, string parameters) : base(taskNode, parameters){
}
...
void run(){
	...
	try{
		...
		log(coreFactory, task, Task.TASK_RUNNING, "Copying Trlp");

		int copied = coreFactory.generateCMSTrlp();
		
		task.setAdicionalData("copied : " + copied.ToString() + " records of trlp table");
