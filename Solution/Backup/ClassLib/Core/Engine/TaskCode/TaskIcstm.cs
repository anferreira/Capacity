...
public 
class TaskIcstm : GenericTaskCode{

public
TaskIcstm(TaskNode taskNode, string parameters) : base(taskNode, parameters){
}
...
void run(){
	...
	try{
		...
		log(coreFactory, task, Task.TASK_RUNNING, "Copying Icstm");

		int copied = coreFactory.generateCMSIcstm();
		
		task.setAdicionalData("copied : " + copied.ToString() + " records of icstm table");
