...
public 
class TaskIcstp : GenericTaskCode{

public
TaskIcstp(TaskNode taskNode, string parameters) : base(taskNode, parameters){
}
...
void run(){
	...
	try{
		...
		log(coreFactory, task, Task.TASK_RUNNING, "Copying Icstp");

		int copied = coreFactory.generateCMSIcstp();
		
		task.setAdicionalData("copied : " + copied.ToString() + " records of icstp table");
