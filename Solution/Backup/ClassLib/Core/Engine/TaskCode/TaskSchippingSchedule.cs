...
public 
class TaskSchippingSchedule : GenericTaskCode{

public
TaskSchippingSchedule(TaskNode taskNode, string parameters) : base(taskNode, parameters){
}
...
void run(){
	...
	try{
		...
		log(coreFactory, task, Task.TASK_RUNNING, "Copying DelJit");

		int copied = coreFactory.generateCMSJitToDelJit();
		
		task.setAdicionalData("copied : " + copied.ToString() + " records of deljit table");
