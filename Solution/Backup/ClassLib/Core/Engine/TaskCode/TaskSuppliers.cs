...
public 
class TaskSuppliers : GenericTaskCode{

public
TaskSuppliers(TaskNode taskNode, string parameters) : base(taskNode, parameters){
}
...
void run(){
	...
	try{
		...
		log(coreFactory, task, Task.TASK_RUNNING, "Copying Suppliers");

		int copied = coreFactory.generateCustVend();
		
		task.setAdicionalData("copied : " + copied.ToString() + " records of CustVend table");
