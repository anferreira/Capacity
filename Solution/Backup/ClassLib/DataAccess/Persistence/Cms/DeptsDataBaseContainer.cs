...
public 
class DeptsDataBaseContainer : GenericDataBaseContainer{

public 
DeptsDataBaseContainer(DataBaseAccess dataBaseAccess) : base(dataBaseAccess){
}
...
void read(){
	...
	try{
		...
		while(reader.Read()){
			DeptsDataBase deptsDataBase = new DeptsDataBase(dataBaseAccess);
			deptsDataBase.load(reader);
			this.Add(deptsDataBase);
		}
