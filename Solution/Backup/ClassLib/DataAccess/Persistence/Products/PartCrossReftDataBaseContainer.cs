...
public 
class PartCrossRefDataBaseContainer : GenericDataBaseContainer{

public 
PartCrossRefDataBaseContainer(DataBaseAccess dataBaseAccess) : base(dataBaseAccess){
}
...
void read(){
	...
	try{
		...
		while(reader.Read()){
			PartCrossRefDataBase partCrossRefDataBase = new PartCrossRefDataBase(dataBaseAccess);
			partCrossRefDataBase.load(reader);
			this.Add(partCrossRefDataBase);
		}
...
void truncate(){
	try{
		string sql = "delete from partcrossref";
