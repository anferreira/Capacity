...
namespace Nujit.NujitERP.ClassLib.Encryption
{
	public class BlowfishSimple
	{
		...
		///		by calling the VerifyKey() method to see if a key
		///		matches for decryption or not.
		/// </remarks> 
		...
		static byte[] TransformKey(String sKey)
		{
		...
		/// </returns> 
		public static bool VerifyKey(String sKey, String sKeyChecksum)
		{
			// decode what we got
		...
		/// </param>
		public BlowfishSimple(String sKey)
		{
		...
		public String Encrypt(String sPlainText)
		{
			...
			// (we need aligned and paddable buffers) 
			...
			nLen--;
			while (nI < nLen) inBuf[nI++] = 0;
			inBuf[nLen] = (byte)nMod;
		...
		public String Decrypt(String sCipherText)
		{
			try{
				...
				if (nDataAbs > 0){
					...
					nOrigSize = 0; // to prevent array out of bounds
				}
				// end of modified by Gustavo Muller
