...
void initializeItemsGrid(){
	if (vecMat == null){
		vecMat = core.getAllMatReqAsString(deptFilter, partFilter);
	}
...
DataSet generateDataSetMaterialsByMaterial(){
	...
	for(int z = 0; z < vecMat.Length; z++){
		...
		if (!lastMaterial.Equals(line[5])){
			...
			Inventory inventory = core.readInventory(line[5]);
...
void dayDueDatebutton_Click(object sender, System.EventArgs e){
	string[][] materialDueDate = core.getMaterialDueDate(false, deptFilter, partFilter, false, matbFilter);
...
void weeksButton_Click(object sender, System.EventArgs e){
	string[][] materialDueDate = core.getMaterialDueDate(true, deptFilter, partFilter, false, matbFilter);
	DataSet dataSet = generateDataSetMaterialDueDate(materialDueDate);
...
void supplierDayButton_Click(object sender, System.EventArgs e){
	string[][] materialDueDate = core.getMaterialDueDate(false, deptFilter, partFilter, true, matbFilter);
	DataSet dataSet = generateDataSetMaterialDueDate(materialDueDate);
...
void supplierWeekButton_Click(object sender, System.EventArgs e){
	string[][] materialDueDate = core.getMaterialDueDate(true, deptFilter, partFilter, true, matbFilter);
	DataSet dataSet = generateDataSetMaterialDueDate(materialDueDate);
